...
- **Location search**: `POST /find-pharmacies`, `POST /search-doctors` — geocode the given `city` via Google Geocoding API, then query Google Places `nearbysearch` (type `pharmacy` / `doctor` with a specialty/urgency-derived keyword) using `GOOGLE_MAPS_API_KEY`.
...
- `pages/` — one file per route (`HomePage`, `SignIn`, `SymptomCheck`, `ContactDoctor`, `Findpharmacy`, `Dashboard`). Most pages still call the backend with a **hardcoded** production URL (`https://ai-health-check.onrender.com/...`) via `axios` or `fetch` — there is no shared API client module. `SymptomCheck.js` is the exception: it reads `process.env.REACT_APP_API_BASE_URL`, falling back to the production URL, so a local `fe/.env.local` (gitignored, not committed) with `REACT_APP_API_BASE_URL=http://localhost:5001` points local dev at a local backend. The other pages haven't been migrated to this pattern yet.
